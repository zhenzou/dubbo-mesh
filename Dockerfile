...
RUN CGO_ENABLED=0 GOOS=linux go build --tags "gojay prod" -o /root/mesh/agent/consumer ./cmd/consumer/main.go
RUN CGO_ENABLED=0 GOOS=linux go build --tags "gojay prod" -o /root/mesh/agent/provider ./cmd/provider/main.go
